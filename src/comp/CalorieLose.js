...
function CalorieLose() {
  ...
  const [Losecalorie,setLosecalorie]=useState('');
  ...
  let calculbmr=(event)=>{
   ...
   let  bmr =(
      ...
      5 * parseInt(age));
     let clr=((bmr*Activity[activity])-500)

     setLosecalorie(clr.toFixed(2))
...
{show && (<h3  className='bg'>Daily Calorie Requirements : { Losecalorie}</h3>)}<br/>

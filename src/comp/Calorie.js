...
function Calorie() {
  ...
  const [Mcalorie,setMcalorie]=useState('');
  ...
  let calculbmr=(event)=>{
   ...
   let  bmr =(
      ...
      5 * parseInt(age));
     let clr=(bmr*Activity[activity])

     setMcalorie(clr.toFixed(2))
...
{show && (<h3  className='bg'>Daily Calorie Requirements : { Mcalorie}</h3>)}<br/>
